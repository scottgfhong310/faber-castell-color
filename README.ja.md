...
- **セット収録対照ページ**（`sets.html`）— 逆方向。公式カラーチャートに倣った色 × セットの一覧表で、一度に 1 シリーズを表示。セットを選ぶと収録色だけが残り、他のセットが網羅しているかを横に見比べられます（例：*skin tones* を選ぶと、50 色セットは 1 色も含まず、100 色セットでも 6 色しか含まないことが一目で分かります）。2 ページは相互にリンクします。
