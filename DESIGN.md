...
（lib，純函式）就是為此存在，UI 是**套組瀏覽 Modal**（側鍵 `widgets`）。

- 兩個 Modal 互為反向且**互相連通**：明細裡的每個尺寸可點 → 開套組瀏覽並停在該套組；
  套組裡的每個色票可點 → 關掉自己、開該色明細。同時只開一個 Modal（避免 Modal 疊 Modal）。
- **選獨立 Modal、不做主網格篩選**（2026-07-28 owner 拍板）：主網格維持 259 色不被改狀態，
  瀏覽完關掉即可。代價是 Modal 內要再刻一套小色票，這是刻意接受的重複。
- **補集也要列**：8 個色不屬於任何套組（114/116/148/220/239/290/292/294，皆為色卡上有色塊
  但套組表未涵蓋的分頁）。不列出來的話，各套組色數永遠加不回 259，看起來像資料漏了。
- **尺寸就是色數**，所以 chip 平常只印尺寸；只有「尺寸 ≠ 實際色數」時才把實際數字以 `≠N`
  顯出來——那是資料壞掉的警訊，不是常態裝飾（反向驗證過：拿掉一個色，該 chip 立刻出現 `≠`）。
- **坑**：補集的偽產品線名用了開頭是空白的 sentinel 會失效——寫進 `data-*` 再讀回來前導空白
  會被正規化掉，比對永遠不中。改用 `__none__`（產品線都是人看的商品名，不會撞名）。
