...
- **本 app 自己的入口（2026-07-30 才補上）**：這座橋做出來是給消費端用的，**本 app 一直沒有 UI**
  ...
  - **系列選單上的數字是實際比對池**（`ag` 135／`black-edition` 106／全部 241），
    ...
    **印總數會與結果對不上**，而使用者數得出來。
  - **UI 的預設是「全部系列」，不是 lib 的 `ag`**（2026-07-30 訂正）。這兩個預設服務的是
    不同的問題，把它們對齊是錯的：
    - lib 的 `ag` 預設服務**消費端**——`color-palette`／`thangka-trace` 問「該拿哪支筆」，
      不該推薦另一條 hobby 線。這個預設不動（上面那條的 4096 點驗證仍然成立）。
    - 本 app 的側欄服務**正在看這面色彩牆的人**，牆上就攤著 259 色。第一版沿用 `ag`，
      結果是**輸入 707 的 hex `#194e8a`，707 不在結果裡**——首名變成 143 cobalt blue（ΔE 7.3）。
      使用者看到的不是「這是 Art & Graphic 的答案」，是「這個比對器壞了」。
    - 旁證：`caran-dache-color`（全系列）與 `copic-color`（全部 358 色）都是「全部」起手，
      **FC 是三支裡唯一預設子集的**。三支的側欄形制既然對齊，預設範圍沒有理由不對齊。
    - 限定系列改成**明示**的動作（選單第二、三項），過濾本身沒有拿掉：切到 `ag` 時
      707 依然不在結果裡，這是刻意保留的能力。
