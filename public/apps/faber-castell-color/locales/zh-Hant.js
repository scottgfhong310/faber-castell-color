/* 繁體中文（zh-Hant） */
I18n.register('zh-Hant', {
  'title.page': 'Faber-Castell 色號 → CSS',
  'app.title': 'Faber-Castell 色號 → CSS',
  'app.sub': '色票取自 Faber-Castell 官方色卡（Art & Graphic ＋ Black Edition），為螢幕近似值、非官方 RGB 規格',

  'search.placeholder': '色號或色名…',
  'search.empty': '找不到符合的顏色',
  'count.showing': '顯示 {n} / {total} 色',

  'tool.sort': '排序：色號 / 色相 / 明度 / 色系 / hex',
  'tool.sets': '依套組瀏覽（套組 → 顏色）',
  'tool.css': '檢視 / 複製整份 CSS',
  'tool.download': '下載 faber_castell_colors.css',
  'tool.mode': '切換 light / dark',
  'tool.lang': '語言',
  'tool.clearFilter': '清除',

  'sort.code': '色號',
  'sort.hue': '色相',
  'sort.lightness': '明度',
  'sort.family': '色系分群',
  'sort.hex': 'hex 原始值',
  'toast.sort': '排序：{m}',

  'family.red': '紅',
  'family.orange': '橙',
  'family.yellow': '黃',
  'family.green': '綠',
  'family.cyan': '青',
  'family.blue': '藍',
  'family.purple': '紫',
  'family.magenta': '洋紅',
  'family.neutral': '中性 / 灰階',

  'detail.approx': '近似',
  'detail.lightfastness': '耐光度',
  'detail.sets': '套組收錄',
  'detail.close': '關閉',
  'sets.ct': ' 色',

  'note.metallic': '金屬色：漸層色票、無單一真值，hex 僅為近似',
  'note.pixel': '像素取樣自 PDF 色票',
  'note.crossValidated': '兩次獨立抽取交叉驗證',
  'note.vector': '取自官方色卡的向量填色（精確值）',

  'title.sets': 'Faber-Castell 套組收錄對照',
  'sets.pageTitle': '套組收錄對照',
  'sets.pageSub': '哪個套組收了哪些色——形制對齊 Faber-Castell 官方色卡的收錄表',
  'sets.baseline': '我已經有的',
  'setlist.title': '選擇系列與套組',
  'setlist.pickSeries': '選擇系列',
  'setlist.pickOwned': '我已經有的套組',
  'setlist.ownedHint': '選一組之後，其餘套組會顯示相對它還能多帶來幾色；再點一次取消。',
  'setlist.none': '（未選，只看收錄）',
  'sets.baselineNone': '（未選，只看收錄）',
  'sets.baselineOn': '差額模式',
  'sets.colColour': '顏色',
  'sets.tableN': '{colours} 色 × {sets} 個套組',
  'sets.emptyRows': '其中 {n} 色不在任何套組裡（色卡有色塊，但收錄表未涵蓋該分頁）',
  'sets.legendIn': '收錄',
  'sets.legendBase': '基準套組',
  'sets.legendOwned': '基準已涵蓋',
  'sets.legendNew': '相對基準的新色',
  'sets.foot': '點欄位的尺寸可直接設為基準（再點一次取消）；點左欄的色號回色票網格。同一條產品線的尺寸是巢狀的，所以有意義的差額都在跨產品線的欄位上。',
  'tool.setlist': '選擇系列與已擁有的套組',
  'tool.grid': '回色票網格',

  'sets.title': '依套組瀏覽',
  'sets.sub': '{lines} 條產品線、{combos} 個套組。點尺寸看它收錄哪些色；點色票看該色明細。',
  'sets.pick': '選一個套組，這裡會列出它收錄的顏色。',
  'sets.none': '未收錄於任何套組',
  'sets.noneHead': '未收錄於任何套組 · {n} 色',

  'series.ag': 'Art & Graphic 系列 · 來源 {source}',
  'series.black-edition': 'Black Edition 系列 · 來源 {source}',

  'css.title': 'CSS 變數 + utility classes',
  'css.sub': '{n} 個 <code>--fc-NNN</code> 變數與對應的 <code>.fc-color-NNN</code> / <code>.fc-bg-NNN</code>',
  'css.copy': '複製全部',
  'css.download': '下載 .css',

  'toast.copied': '已複製：{v}',
  'toast.copyFail': '複製失敗（需 localhost 或 HTTPS）',
  'toast.cssCopied': '已複製整份 CSS',
  'toast.downloaded': '已下載 {n}',
  'toast.lang': '已切換為 {name}'
}, '繁體中文');
