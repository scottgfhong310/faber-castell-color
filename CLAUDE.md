...
- **可嵌入 lib** `faber-castell-color-lib.js`（`window.FaberCastellCssLib`）：`filter` / `sortColors`
  （`code`/`hue`/`lightness`，無彩度殿後）/ `hexToRgb` / `rgbToHsl` / `pickTextColor`（WCAG 對比選黑白字）/
  `copyValue` / **`buildCss`（產生整份 `:root`＋utility）**，**純邏輯不碰 DOM**；
  `faber-castell-color.js` 才是碰 DOM 的控制器（渲染、排序側鍵、Modal、clipboard、toast）。
